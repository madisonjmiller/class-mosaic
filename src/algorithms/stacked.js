export function createStackedSchedule(classData) {
	...
	while (currentCredits < maxCredits && optionalClasses.length > 0) {
		...
		optionalClasses.sort((a, b) => {
			const score = (classItem) => {
				...
				const totalScore = overlap * 10 - newDays;
				return totalScore;
			};
